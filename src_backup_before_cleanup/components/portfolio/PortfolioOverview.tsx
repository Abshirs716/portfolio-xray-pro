...
export const PortfolioOverview = ({ className = "" }: PortfolioOverviewProps) => {
  ...
  useEffect(() => {
    if (metrics && portfolio) {
      ...
      console.log('💰 REAL portfolio total value:', portfolio.total_value);
      
      // Calculate actual dollar amounts from percentages
      const totalValue = portfolio.total_value || metrics.totalValue || 0;
