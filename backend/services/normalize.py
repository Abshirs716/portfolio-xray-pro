...
ALIASES: Dict[str, List[str]] = {
    ...
    "price": ["price","unitprice","shareprice","pricepershare","lastprice","closeprice","marketprice","currentprice","nav","px","close","last"],
...
def detect_custodian(sample_lines: List[str], filename: str = "") -> str:
    ...
    custodian_patterns = [
        ...
        ("Fidelity", ["fidelity"]),
        ("TD Ameritrade", ["td ameritrade", "tdameritrade", "ameritrade", "td-", "td_"]),
        ("Vanguard", ["vanguard"]),
...
def build_parse_result(
    ...
    all_custodians: List[str] = []  # Collect all unique custodians
    ...
    for (fname, blob) in file_bytes_list:
        ...
        # Collect custodians (skip Unknown/Custom)
        if custodian not in ["Unknown", "Custom Format"] and custodian not in all_custodians:
            all_custodians.append(custodian)
        ...
        print(f"DEBUG: Custodian detected: {custodian}")
    ...
    # Format custodian display
    if len(all_custodians) == 0:
        custodian_display = "Unknown"
    elif len(all_custodians) == 1:
        custodian_display = all_custodians[0]
    else:
        # Multiple custodians - show them all
        custodian_display = f"Multiple ({len(all_custodians)}): {', '.join(all_custodians)}"

    print(f"DEBUG: Final custodian display: {custodian_display}")
    ...
    return {
        ...
        "metadata": {
            "custodianDetected": custodian_display,
            "confidence": 95 if combined_holdings else 30,
            "files": file_previews,
            "transparency_score": 0.0,
            "batch_mode": True,
            "all_custodians": all_custodians,  # Also keep list for detailed info
        }
    }